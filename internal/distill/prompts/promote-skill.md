...
A good description is **specific**: it names the situations that should trigger the skill. Bad: "for code quality". Good: "Use when writing or reviewing TypeScript/full-stack code. Covers …" If the observation is project-scoped, mention the project/workflow trigger rather than making the skill sound globally applicable.
...
If the observation is too thin to make a useful skill, say so by setting the body to `INSUFFICIENT` and skip — caller will surface that. Project-specific patterns are valid when the skill description clearly names the project-scoped trigger.
